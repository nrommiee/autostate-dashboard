...
const StatisticsCardWithSvg = ({ title, badgeContent, value, changePercentage, svg, className, loading = false }: StatisticsCardProps) => {
  return (
    <Card className={cn('relative justify-between gap-6', className)}>
      <CardHeader className='flex flex-col gap-3'>
        <span className='font-medium'>{title}</span>
        <Badge className='bg-primary/10 text-primary hover:bg-primary/10'>{badgeContent}</Badge>
      </CardHeader>
...
export default StatisticsCardWithSvg
